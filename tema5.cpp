...
public:
    ...
    void adauga_pixel2(int i, int j) {
        if(i > this->linii || i < 0 )
            return;
        if(j > this->coloane || j < 0)
            return;
    ...
    int AfisarePuncteCerc3 (int x, int y, vector<pair<int,int>>&M) {
        M.clear();
    ...
    void AfisareCerc4(int raza) {
        ...
        int num = AfisarePuncteCerc3(x, y, M);
...
void Display6() {
    ...
    glLoadIdentity();
    gc.deseneaza();
    gc.adauga_pixel2(0, 0);
    gc.adauga_pixel2(0, 1);
    glPopMatrix();
}
